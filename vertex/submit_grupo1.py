"""
submit_grupo1.py
Empaqueta el trainer y lanza el Custom Job en Vertex AI para el Grupo 1.
...
PACKAGE_GCS   = f"gs://{BUCKET_NAME}/packages/trainer_grupo1-0.1.tar.gz"
...
def build_package():
    print("Empaquetando trainer grupo 1...")
    subprocess.run(
...
def upload_package():
    ...
    bucket.blob("packages/trainer_grupo1-0.1.tar.gz").upload_from_filename(PACKAGE_LOCAL)
    print("Paquete subido.")


def submit_job():
    print("Enviando Custom Job (Grupo 1) a Vertex AI...")
    ...
    bootstrap = (
        ...
        "python -m trainer.task_grupo_1"
    )

    job = aiplatform.CustomJob(
        display_name="tfm_grupo1",
        worker_pool_specs=[{
            "machine_spec": {
    ...
    print(f"\nResultados en:")
    print(f"  gs://{BUCKET_NAME}/grupo_1/1_1/ ... gs://{BUCKET_NAME}/grupo_1/1_6/")
