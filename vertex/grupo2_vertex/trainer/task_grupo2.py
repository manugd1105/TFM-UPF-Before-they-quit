"""
trainer/task_grupo2.py
Vertex AI Custom Job — Group 2 executions (all features + PCA, 2-1 to 2-19).
...
FEATURE_SETS = {"group2": FEATURES_GROUP2}
...
# ─── Executions ───────────────────────────────────────────────
EXECUTIONS = [
    ("2-1",  "group2", "1_dim",   "elasticnet"),
    ("2-2",  "group2", "5_dim",   "elasticnet"),
    ("2-3",  "group2", "1_per_5", "elasticnet"),
    ("2-4",  "group2", "1_dim",   "knn"),
    ...
    ("2-10", "group2", "1_dim",   "random_forest"),
    ("2-11", "group2", "5_dim",   "random_forest"),
    ("2-12", "group2", "1_per_5", "random_forest"),
...
def run_execution(exec_id, feature_group, pca_mode, model_type, train_raw, test_raw):
    ...
    print(f"  Training {model_type} ({n_iter} iter x {N_FOLDS} folds, {len(train_x.columns)} features after PCA)...")
    t0         = datetime.now()
