"""
trainer/task_grupo6.py
Vertex AI Custom Job — Group 6 executions (LightGBM, all feature/PCA combos, 6-1 to 6-13).
Reads data from gs://<GCP_BUCKET>, saves results to gs://<GCP_BUCKET>/grupo_6/
"""
...
_JDI_ODI_PREFIXES = ["pros_sim_ODI_","cons_sim_ODI_","pros_sim_JDI_","cons_sim_JDI_"]

FEATURES_GROUP1 = [
    "summary_charlen","summary_wordlen","pros_charlen","pros_wordlen","cons_charlen","cons_wordlen",
]
FEATURES_GROUP3 = [f for f in FEATURES_ALL if any(f.startswith(p) for p in _NLP_PREFIXES)]
FEATURES_GROUP4 = [f for f in FEATURES_ALL if any(f.startswith(p) for p in _NLP_PREFIXES + _JDI_ODI_PREFIXES)]

FEATURE_SETS = {
    "group1": FEATURES_GROUP1,
    "group2": FEATURES_ALL,   # all features, used with PCA
    "group3": FEATURES_GROUP3,
    "group4": FEATURES_GROUP4,
    "group5": FEATURES_ALL,   # all features, no PCA
}
...
# ─── Executions ───────────────────────────────────────────────
# 6-1:  string length features, no PCA
# 6-2 to 6-4:  all features + 3 PCA modes  (mirrors group 2)
# 6-5 to 6-8:  NLP features + no PCA + 3 PCA modes  (mirrors group 3)
# 6-9 to 6-12: NLP+JDI/ODI + no PCA + 3 PCA modes  (mirrors group 4)
# 6-13: all features, no PCA  (mirrors group 5)
EXECUTIONS = [
...
def run_execution(exec_id, feature_group, pca_mode, train_raw, test_raw):
    ...
    print(f"  Training lightgbm ({n_iter} iter x {N_FOLDS} folds, {len(train_x.columns)} features)...")
    t0         = datetime.now()
...
if __name__ == "__main__":
    ...
    if completed:
        print(f"Skipping {len(completed)} already-completed executions: {sorted(completed)}")
    ...
    print(f"\nGlobal summary -> gs://{BUCKET_NAME}/{OUTPUT_PREFIX}/model_summary.txt")

    print("\n=== FINAL RESULTS ===")
    for r in results:
        if "error" in r:
            print(f"  {r['id']} (lightgbm): ERROR - {r['error']}")
        else:
            print(f"  {r['id']} (lightgbm): MAPE={r['mape_test']:.4f} | MSE={r['mse_test']:.4f}")
