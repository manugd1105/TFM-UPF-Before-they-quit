"""
trainer/task_grupo7.py
Vertex AI Custom Job — Group 7: higher n_iter re-runs.
  7-1  RF          group4 (NLP+JDI/ODI)   no PCA   n_iter=100
  7-2  RF          group5 (all features)  no PCA   n_iter=100
  7-3  XGBoost     group4                 no PCA   n_iter=200
  7-4  XGBoost     group5                 no PCA   n_iter=200
  7-5  LightGBM    group4                 no PCA   n_iter=200
  7-6  LightGBM    group5                 no PCA   n_iter=200
"""
...
# ─── Feature sets ─────────────────────────────────────────────
_NLP_PREFIXES = [
    "summary_charlen", "summary_wordlen", "pros_charlen", "pros_wordlen",
    "cons_charlen", "cons_wordlen",
    "summary_ld_", "pros_ld_", "cons_ld_",
    "pros_emolex_", "cons_emolex_",
    "pros_nltk_", "cons_nltk_",
    "ODI_related_vocab_",
    "pros_empath_", "cons_empath_",
]
_SIM_PREFIXES = ["pros_sim_", "cons_sim_"]
...
# ─── Executions ───────────────────────────────────────────────
EXECUTIONS = [
    ("7-1", "group4", "random_forest"),
    ("7-2", "group5", "random_forest"),
    ("7-3", "group4", "xgboost"),
    ("7-4", "group5", "xgboost"),
    ("7-5", "group4", "lightgbm"),
    ("7-6", "group5", "lightgbm"),
...
def get_model(model_type):
    if model_type == "random_forest":
        return RandomForestRegressor(random_state=RANDOM_SEED, n_jobs=N_JOBS)
    if model_type == "xgboost":
        return xgb.XGBRegressor(random_state=RANDOM_SEED, n_jobs=N_JOBS)
    if model_type == "lightgbm":
        return lgb.LGBMRegressor(random_state=RANDOM_SEED, n_jobs=1, verbose=-1)
...
def run_execution(exec_id, feature_group, model_type, train_raw, test_raw):
    ...
    n_jobs_cv = 1 if model_type == "lightgbm" else N_JOBS
...
if __name__ == "__main__":
    ...
    if completed:
        print(f"Skipping {len(completed)} already-completed: {sorted(completed)}")

    results = []
    for exec_id, feature_group, model_type in EXECUTIONS:
        if exec_id in completed:
            print(f"SKIP {exec_id} — already in GCS")
            continue
        try:
            mape, mse = run_execution(exec_id, feature_group, model_type, train_raw, test_raw)
