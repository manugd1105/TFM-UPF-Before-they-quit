"""
trainer/task_grupo4.py
Vertex AI Custom Job — Group 4 executions (NLP + JDI/ODI similarities, 4-1 to 4-24).
SVR limited to 5 iter / max_iter=500 to avoid multi-day runtimes.
"""
...
N_ITER = {
    ...
    "decision_tree": 40,
    "random_forest": 40,
    "svr":           10,   # 10 iter × 5 folds = 50 fits (~2-7h)
    "xgboost":      100,
}
N_FOLDS     = 5
N_JOBS      = 16
...
# ─── Feature sets ─────────────────────────────────────────────
# Group 4: NLP features + JDI/ODI semantic similarities
# (Group 3 NLP features plus the sim_ODI_* and sim_JDI_* columns)
_NLP_PREFIXES = [
...
# Full feature list (same as FEATURES_GROUP2 from grupo3)
FEATURES_ALL = [
...
FEATURE_SETS = {"group4": FEATURES_GROUP4}
...
def run_execution(exec_id, feature_group, pca_mode, model_type, train_raw, test_raw):
    print(f"\n{'='*60}")
    ...
    print(f"  Training {model_type} ({n_iter} iter x {N_FOLDS} folds)...")
    t0         = datetime.now()
...
if __name__ == "__main__":
    ...
    for exec_id, fg, pca_mode, model_type in EXECUTIONS:
        if exec_id in completed:
            ...
            continue
        try:
            mape, mse = run_execution(exec_id, fg, pca_mode, model_type, train_raw, test_raw)
