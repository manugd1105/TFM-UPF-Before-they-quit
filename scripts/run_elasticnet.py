"""
run_elasticnet_knn.py
---------------------
Runs ALL Elasticnet executions (all groups, all PCA configs).
Repeats everything — does not skip already completed runs.

Iterations:
  - Elasticnet: 13  (covers ~52% of 25 combinations)
"""
...
# ============================================================
# EXECUTIONS — ALL Elasticnet (all groups)
# ============================================================
EXECUTIONS = [
    # Group 1
    ("1-1", "group1", None,      "elasticnet"),
...
def run_execution(exec_id, feature_group, pca_mode, model_type):
    ...
    n_iter   = min(N_ITER[model_type], 1)
    for v in params.values(): n_iter = max(n_iter, 1)
    grid_size = 1
    ...
    search = RandomizedSearchCV(
        ...
        cv=N_FOLDS, random_state=RANDOM_SEED, n_jobs=N_JOBS, verbose=1)
...
if __name__ == "__main__":
    print(f"Running {len(EXECUTIONS)} Elasticnet executions (all groups)")
